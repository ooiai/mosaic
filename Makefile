...
# Usage: make cli-release-assets v=v0.2.0-beta.3 t=aarch64-apple-darwin
cli-release-assets:
	@if [ -z "$(v)" ] || [ -z "$(t)" ]; then \
		echo "error: missing args. usage: make cli-release-assets v=v0.2.0-beta.3 t=aarch64-apple-darwin"; \
		exit 1; \
	fi
...
# Usage: make cli-release-manifests v=v0.2.0-beta.3 assets=dist/v0.2.0-beta.3 out=dist/v0.2.0-beta.3
cli-release-manifests:
	@if [ -z "$(v)" ] || [ -z "$(assets)" ]; then \
		echo "error: missing args. usage: make cli-release-manifests v=v0.2.0-beta.3 assets=dist/v0.2.0-beta.3 [out=dist/v0.2.0-beta.3]"; \
		exit 1; \
	fi
