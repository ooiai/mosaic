...
| `core agent/agents` | 94% | deeper agent/session management ergonomics and richer multi-agent controls beyond current chat/TUI switching flows |
...
| `agents` | `agents list/current/add/update/show/remove/default/route` + route/default-driven runtime selection + CLI-side resolution diagnostics via `agents current` + inline default/route visibility in `agents list`, `chat /agents`, and the TUI picker + session stickiness on resume + in-conversation agent switching through `chat /agent <id>`, interactive `tui /agent <id>`, the TUI `Ctrl+A` agent picker, `Ctrl+S` session picker, and `/session <id>` `/new` `/status` input commands + TUI session selection rebinding to the selected session's runtime metadata | partial | 95% |
| `sessions` | `session list/show/resume/clear` + alias `sessions` + runtime metadata exposure (`runtime.profile_name`, `runtime.agent_id`) + dedicated regression coverage for list/show/resume/clear + runtime summary in `session list` | partial | 89% |
...
| `tui` | `tui` chat-first terminal UI (`mosaic-tui` + `ratatui/crossterm`) with sessions/messages/inspector panes, focus control (`--focus`), inspector toggle (`--no-inspector`), interactive keymap, non-interactive `--prompt` JSON mode compatibility, session-bound agent resume, slash-command runtime control (`/agents`, `/agent`, `/session`, `/new`, `/status`) with safe session reset, TUI `Ctrl+A` agent picker, `Ctrl+S` session picker, session-list rebinding to the selected session runtime, visible per-session profile/agent summaries, and always-visible runtime status line context | partial | 100% |
