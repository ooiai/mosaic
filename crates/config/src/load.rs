...
pub fn default_config_template() -> Result<String> {
    let body = serde_yaml::to_string(&init_config_template(
        &InitWorkspaceConfigOptions::default(),
    )?)?;
    Ok(format!(
...
pub fn init_workspace_config(
    cwd: impl AsRef<Path>,
    options: &InitWorkspaceConfigOptions,
) -> Result<PathBuf> {
    let cwd = cwd.as_ref();
    ...
    let config = init_config_template(options)?;
...
fn init_config_template(options: &InitWorkspaceConfigOptions) -> Result<MosaicConfig> {
    ...
    if !config.profiles.contains_key(&active_profile) {
        let available = config
            ...
            .join(", ");
        bail!(
            "unknown setup profile '{}'; expected one of {}",
            active_profile,
            available
        );
    }
