    ...
    pub(crate) fn plan_route(&self, req: &RunRequest) -> Result<PlannedRoute> {
        ...
        if let Some(tool_name) = req.tool.as_ref() {
            ...
            return Ok(PlannedRoute::Tool {
                ...
                source_name: Self::tool_source_name(tool.metadata()),
                source_path: Self::tool_source_path(tool.metadata()),
                source_version: Self::tool_source_version(tool.metadata()),
        ...
        if let Some(skill_name) = req.skill.as_ref() {
            ...
            return Ok(PlannedRoute::Skill {
                ...
                source_path: Self::skill_source_path(skill.metadata()),
        ...
        if let Some(workflow_name) = req.workflow.as_ref() {
            ...
            return Ok(PlannedRoute::Workflow {
                ...
                source_name: Self::workflow_source_name(&workflow.metadata),
                source_path: Self::workflow_source_path(&workflow.metadata),
                source_version: Self::workflow_source_version(&workflow.metadata),
    ...
    fn auto_route_workflow(
        ...
        self.ctx.workflows.list().into_iter().find_map(|name| {
            ...
            .map(|reason| PlannedRoute::Workflow {
                ...
                source_name: Self::workflow_source_name(&registered.metadata),
                source_path: Self::workflow_source_path(&registered.metadata),
                source_version: Self::workflow_source_version(&registered.metadata),
                execution_target: Self::workflow_execution_target(),
